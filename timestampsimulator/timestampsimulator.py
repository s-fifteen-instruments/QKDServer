...
def simulate_pair_source_timestamps_write_into_two_files(file_name_ph1, file_name_ph2, 
                                                        ...
                                                        rate_pairs, tot_time, pattern_photon_1=int('0001', 2), 
...
def simulate_entangled_pair_source(file_name_ph1, file_name_ph2, rate_photon_1, rate_photon_2, rate_pairs, tot_time, pair_delay=1e-6):
    ...
    arrival_times_photon_1 = np.random.exponential(
        1 / rate_photon_1, int(rate_photon_1 * tot_time)).cumsum()
    arrivals_times_photon_2 = np.random.exponential(
        1 / (rate_photon_2 - rate_pairs), int((rate_photon_2 - rate_pairs) * tot_time)).cumsum()
    # select random events in the channel 1 and add a partner photon in channel 2
    heralding_efficiency = rate_pairs / rate_photon_1
    mask_pair_events = np.random.binomial(1, heralding_efficiency, len(
        arrival_times_photon_1)).astype(bool)  # selects randomly events to create pairs

    # create uncorreclated photons for photon 1 they end up on a random detector equally distributed
    uncorr_detections_photon_1 = arrival_times_photon_1[np.logical_not(
        mask_pair_events)]
    random_channel = 0b1 << np.random.randint(
        0, 4, len(uncorr_detections_photon_1))
    uncorr_time_pattern_photon1 = np.vstack(
        [uncorr_detections_photon_1, random_channel]).T

    random_channel = 0b1 << np.random.randint(
        0, 4, len(arrivals_times_photon_2))
    uncorr_time_pattern_photon_2 = np.vstack(
        [arrivals_times_photon_2, random_channel]).T
    ...
    pair_times_photons_basis2 = pair_times_photon1[np.logical_not(
        mask_basis1_meas)]

    # BASIS 1 (H/V basis) measurements
    # 50% of the photons are H and the others a V
    mask_H = np.random.binomial(
        1, 1 / 2, len(pair_times_photons_basis1)).astype(bool)
    H_times = pair_times_photons_basis1[mask_H]  # NOTE: photon 1 times
    V_times = pair_times_photons_basis1[np.logical_not(
        mask_H)]  # NOTE: photon 1 times
    # BASIS 2 (A/AD) measurements: photon 1 goes into the basis 2 arm. The photon pair state is HH + VV = DD + AD AD
    mask_D = np.random.binomial(
        1, 1 / 2, len(pair_times_photons_basis2)).astype(bool)
    ...
    H_AD_times = H_basis2_times[np.logical_not(
        mask_H_D)] + pair_delay  # NOTE: photon 2 times
    ...
    V_AD_times = V_basis2_times[np.logical_not(
        mask_V_D)] + pair_delay  # NOTE: photon 2 times
    ...
    AD_H_times = AD_basis1_times[mask_AD_H] + \
        pair_delay  # NOTE: photon 2 times
    AD_V_times = AD_basis1_times[np.logical_not(
        mask_AD_H)] + pair_delay  # NOTE: photon 2 times
    ...
    time_pattern_photon2 = np.vstack(
        [uncorr_time_pattern_photon_2, a, b, c, d, e, f, g, h, i, j, k, l])
