...
def create_det_layout(detector):
    ...
    return html.Div([dbc.Card([dbc.CardHeader(html.H4(det_id)),
                               dbc.CardBody([
                                   dbc.Row([dbc.Col(
...
def get_dev_list():
    ...
    if not dev_list:
        dev_list = glob.glob('/dev/serial/by-id/*SPD*')
    return dev_list


dev_list = get_dev_list()
det_dict = {det.identity(): det for det in [
    SinglePhotonDetector(dev) for dev in dev_list]}
create_callbacks()
...
def serve_layout():
    ...
    hidden_field_det = html.Div(
        '.', id='hidden-div-det', style={'display': 'none'})
