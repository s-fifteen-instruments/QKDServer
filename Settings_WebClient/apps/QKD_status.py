...
# QBER plot
qber_display = html.Div(
    children=['Current quantum bit error: ', html.Nobr(children='---', id='qber')])
ber_display = html.Div(
    children=['Bit error: ', html.Nobr(children='---', id='ber')])
...
connection_status = html.Div(children=['Connection status: ', html.Nobr(
    children='---', id='connection_status')])
symmetry_status = html.Div(
    children=['Symmetry: ', html.Nobr(children='---', id='symmetry')])
protocol_status = html.Div(
    children=['Protocol: ', html.Nobr(children='---', id='protocol')])
tracked_time_diff_status = html.Div(children=[
                                    'Tracked time difference: ', html.Nobr(children='---', id='tracked_time_diff')])
received_epoch_status = html.Div(children=['Last received epoch: ', html.Nobr(
    children='---', id='last_received_epoch')])
init_time_diff_status = html.Div(children=[
                                 'Initial time difference: ', html.Nobr(children='---', id='init_time_diff')])
long_match_status = html.Div(
    children=['pfind long match (in sig): ', html.Nobr(children='---', id='sig_long')])
short_match_status = html.Div(
    children=['pfind short match (in sig): ', html.Nobr(children='---', id='sig_short')])
coincidences = html.Div(children=['coincidences per epoch: ', html.Nobr(
    children='---', id='coincidences')])
accidentals = html.Div(children=['accidentals per epoch: ', html.Nobr(
    children='---', id='accidentals')])
...
total_ec_bits = html.Div(children=[
                         'Total error-corrected bits: ', html.Nobr(children='', id='total_ec_key_bits')])

to_ec = [html.Div(children=['Start epoch: ', html.Nobr(children='', id='first_epoch')]),
         html.Div(children=['Number of epochs: ', html.Nobr(
             children='', id='undigested_epochs')]),
         html.Div(children=['Initial quantum bit error: ',
                            html.Nobr(children='', id='init_QBER')]),
...
from_ec = [html.Div(children=['Key file name: ', html.Nobr(children='', id='key_file_name')]),
           html.Div(children=['Error fraction: ', html.Nobr(
               children='', id='ec_err_fraction')]),
...
# used for loading data at the page load
dump_output_field2 = html.Div('.', id='dump2', style={'display': 'none'})
...
def update_graph_qber(n):
    ...
    fig.add_trace(go.Scatter(x=x, y=np.ones(len(y))*12,
                             mode='lines', name='Max. allowed QBER'))
    # if len(y) > 1:
